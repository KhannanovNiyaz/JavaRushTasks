...
public class Solution {
    ...
    public static void main(String[] args) {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(System.in));
            ...
            String fileDate;
            while ((fileDate = fileReader.readLine()) != null) {
                String[] buf = fileDate.split(" ");
                String outputString = "";
                for (int i = 0; i < buf.length; i++) {
                    if (buf[i].matches("\\d+") && map.containsKey(Integer.parseInt(buf[i]))) {
                        buf[i] = map.get(Integer.parseInt(buf[i]));
                        if (outputString.equals("")) outputString += buf[i]+ " ";
                        else outputString = outputString.concat(buf[i]+ " ");
                        continue;
                    }
                    if (outputString.equals("")) outputString += buf[i]+ " ";
                    else outputString= outputString.concat(buf[i] + " ");
