...
public class Solution {
    ...
    public static byte[] getNetAddress(byte[] ip, byte[] mask) {
        byte[] newIPArray = new byte[4];
        for (int i = 0; i < ip.length; i++) {
            newIPArray[i] = (byte) (ip[i] & mask[i]);
        }
        return newIPArray;
    }
...
//            System.out.format("%08d ", Integer.parseInt(Integer.toBinaryString((int) aByte)));
            System.out.format("%08d ", Integer.parseInt(Integer.toBinaryString(aByte&255|256).substring(1)));
