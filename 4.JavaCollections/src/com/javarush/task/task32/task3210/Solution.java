...
public class Solution {
    public static void main(String... args) throws IOException {
        ...
        long positionNum = Long.parseLong(args[1]);
        String text = args[2];

        RandomAccessFile randomAccessFile = new RandomAccessFile(fileName, "rw");

        randomAccessFile.seek(positionNum);

        byte[] readBuf = new byte[text.length()];
        randomAccessFile.read(readBuf, 0, readBuf.length);

        String readText = new String(readBuf);
